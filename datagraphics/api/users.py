...
def all():
    ...
    for user in datagraphics.user.get_users():
        users.append(
            {
                "href": flask.url_for(
                    "api_user.serve", username=user["username"], _external=True
                ),
                "username": user["username"],
            }
        )
    return utils.jsonify(
        {"users": users}, schema=flask.url_for("api_schema.users", _external=True)
    )
...
schema = {
    ...
    "properties": {
        ...
        "users": {"type": "array", "items": schema_definitions.link},
    },
